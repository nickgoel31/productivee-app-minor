...
import { Check, ChevronDown, ChevronRight, Delete, Dot, Edit, ListChecks, ListTodo, Trash, Trash2, Trash2Icon, Undo } from 'lucide-react'
                    ...
                    {data.map((subtask) => (
                        <div  key={subtask.id} className='w-full  flex cursor-pointer items-center justify-between gap-3 hover:bg-neutral-400/10 transition'>
                            ...
                            <li className='flex items-center  w-full  px-2'>
                                ...
                                <p className={cn('text-sm', subtask.isDone && "line-through text-neutral-500")}>{subtask.title}</p>
                            ...
                            <div className='flex items-center gap-1 pr-2 '>
                                {!subtask.isDone ? (
                                    <Button onClick={() => {
                                        updateSubTaskInDB(subtask.id, true);
                                    }} variant={"ghost"} className='px-2'>
                                        <Check size={15}/>
                                    </Button>
                                ) : (
                                    <Button onClick={() => {
                                        updateSubTaskInDB(subtask.id, false);
                                    }} variant={"ghost"} className='px-2'>
                                        <Undo size={15}/>
                                    </Button>
                                )}
