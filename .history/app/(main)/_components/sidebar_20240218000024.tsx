...
const Sidebar = ({workspaces}: WorkspaceListProps) => {
  ...
  return (
    ...
    {
      !isWhiteboard && (
        <div className=' w-64 bg-background  h-full gap-12 flex flex-col items-center justify-between px-6 py-14 shrink-0 overflow-hidden z-[20]'>
        <div className='w-full h-full flex flex-col items-center shrink-0 gap-10'>
          ...
          <ul className='flex flex-col gap-3 items-start h-full justify-between  w-full'>
                ...
                <AccordionItem value='1' className='border-none w-full '>
                  ...
                  <AccordionContent className='flex flex-col px-2 gap-3'>
                    {workspaces.map((workspace) => (
                      <Link key={workspace.id} className='hover:underline' href={`/workspace/${workspace.id}`}>{workspace.name}</Link>
                    ))}
                    {workspaces.length === 0 && (
              ...
              <div className='flex flex-col  justify-center gap-4'>
                ...
                <Link href={"mailto:harshgoel2004@gmail.com?subject=Have an Idea for Productive App"} className='flex items-center gap-2 text-sm font-medium hover:underline underline-offset-2 transition'><MailIcon size={18}/>{"Have an idea? Email me"}</Link>
              </div>
          </ul>
        </div>
    </div>
