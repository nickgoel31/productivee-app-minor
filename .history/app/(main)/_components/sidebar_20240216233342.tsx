...
const Sidebar = ({workspaces}: WorkspaceListProps) => {
  ...
  return (
    ...
    {
      !isWhiteboard ? (
        <div className='w-64 bg-background border-r fixed left-0 top-0 h-full gap-12 flex flex-col items-center justify-between px-6 py-14 shrink-0 overflow-hidden z-[20]'>
        ...
        <div className='w-20 bg-background border-r h-full gap-12 flex flex-col fixed top-0 left-0 items-center justify-between px-6 py-14 shrink-0 overflow-hidden z-[20]'>
          <div className='w-full h-full flex flex-col items-center shrink-0 gap-10'>
            <Link href={"/"} className='font-semibold text-2xl'>P</Link>
          </div>
        </div>
      )
    }
