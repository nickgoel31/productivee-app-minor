...
const TodoCard = ({id, title, isDone}:{id:string,title:string, isDone:boolean}) => {
    ...
    const onClick = () => {
        if(!isDone){
            ...
            toast({
                title: `Todo Item ${title} marked completed!`,
              })
        }
        else{
            markTodoNotDone(id);
            toast({
                title: `Todo Item ${title} unmarked!`,
              })
        }
    }
  ...
  return (
    <div className='flex items-center justify-between border rounded-md px-4 py-4 w-96 relative'>
        <div className='flex items-center gap-3'>
            <Checkbox />
            <h2 className={cn('font-medium text-lg line-clamp-1', isDone ? "line-through text-neutral-500" : "")}>{title}</h2>
        </div>
        <div className=''>
            <Switch onClick={onClick} className='h-4 w-7'/>
            <Toaster />
        </div>
    </div>
  )
