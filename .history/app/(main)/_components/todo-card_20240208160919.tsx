...
import { Check, ChevronDown, ChevronRight, Delete, Dot, Edit, ListChecks, ListTodo, Trash, Trash2 } from 'lucide-react'
    ...
    <div className='flex flex-col  gap-3 border rounded-md px-4 py-4 w-[600px] relative transition'>
        ...
        {todoOpen && (
            <div className='flex flex-col gap-2 px-7 transition'>
                ...
                <h3 className={cn('text-sm font-medium')}>Subtasks</h3>

                <div className='flex flex-col gap-1 text-neutral-200'>
                    {data.map((subtask) => (
                        <div onClick={() => {}} key={subtask.id} className='w-full py-1 px-2 flex cursor-pointer items-center justify-between gap-3 hover:bg-neutral-400/10 transition'>
