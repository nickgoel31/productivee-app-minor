...
const NewWorkspaceBox = ({workspaces} : WorkspaceListProps) => {
    ...
    function toggleSelectedImg(con:boolean) {
      setSelectedImg(!con)
    }
    ...
    const handleBannerImageClick = (url: string,id:number) => {
      setBannerImageUrl(url)
      toggleSelectedImg(!selectedImg)
    }
            ...
            <div className='flex flex-col space-y-3 mb-1'>
              ...
              <div className='grid grid-cols-3 gap-4 px-1'>
                {bannerImages.map((img) => (
                  <div onClick={() => handleBannerImageClick(img.imageUrl)} key={img.id} className='w-full flex-grow-0 object-contain cursor-pointer flex items-center justify-center group relative overflow-hidden'>
