            ...
            <div className='flex flex-col space-y-3 mb-1'>
              ...
              <div className='grid grid-cols-3 gap-4 px-1'>
                {bannerImages.map((img) => (
                  <div key={img.id} className='w-full flex-grow-0 object-contain cursor-pointer flex items-center justify-center group relative overflow-hidden'>
                    <Image src={img.imageUrl} alt='Banner image' width={1000} height={1000} className='object-contain transition'/>
                    <span className='w-full h-full absolute group-hover:opacity-100 bg-background/50 opacity-0 transition top-0 left-0 text-center flex items-center justify-center text-sm font-medium'>{img.label}</span>
                  </div>
                ))}
              </div>
            </div>
