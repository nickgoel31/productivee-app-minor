...
import { CheckCheck, ChevronDown, ChevronRight, Edit, Edit2, Edit3, Trash2, Undo, Undo2, UndoDot } from 'lucide-react'
...
import EditLegacyTask from './edit-legacy-task'
import DeleteLegacyTask from './delete-leg-task'
    ...
    <div className='flex flex-col w-full border rounded p-2 px-4 gap-2'>
        <div className=' w-full  flex items-center justify-between '>
            <div className='flex items-center gap-1 cursor-pointer'>
                {isTaskExpanded ? (
                    <div onClick={() => setIsTaskExpanded(!isTaskExpanded)} className='p-1 cursor-pointer hover:bg-neutral-500/15 transition rounded'>
                        <ChevronDown size={21}/>
                    </div>
                ) : (
                    <div onClick={() => setIsTaskExpanded(!isTaskExpanded)} className='p-1 cursor-pointer hover:bg-neutral-500/15 transition rounded'>
                ...
                <div className='flex items-center gap-2'>
                    <h6 onClick={() => setIsTaskExpanded(!isTaskExpanded)} className={cn('font-medium select-none', task.isDone && "line-through text-neutral-500")}>{task.title}</h6>
                    <EditLegacyTask taskName={task.title} taskId={task.id}/>
                </div>
            </div>
            ...
            <div className='flex items-center gap-3'>
                ...
                <DeleteLegacyTask taskName={task.title} taskId={task.id}/>
            </div>
        </div>

        {isTaskExpanded && (
            <div className='flex flex-col px-3 lg:px-10 pb-2 w-full gap-2'>
