...
const EditKanbanCardForm = ({prevTitle}:{prevTitle:string}) => {
    ...
    const handleTags = (e: MouseEvent<HTMLButtonElement, MouseEvent>) => {
                    ...
                    <FormItem>
                        ...
                        <FormControl>
                            ...
                            <ToggleGroup type='multiple' className='flex items-end justify-start flex-wrap mid space-y-1'>
                                {defaultTags.map((tag) => (
                                    <ToggleGroupItem onClick={(e) => {handleTags(e)}} value={tag.value} size={"sm"} className='text-xs text-neutral-300 font-medium border rounded-full px-3 capitalize'>{tag.value}</ToggleGroupItem>
                                ))}
                            </ToggleGroup>
                        </FormControl>
                        <FormMessage />
