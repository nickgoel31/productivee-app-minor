...
const KanbanSubtaskCard = ({subtask,workspaceId}:Props) => {
  ...
  return (
    <motion.div layout layoutId={subtask.id} className={cn('max-w-screen-lg border py-2 px-4 rounded flex items-center justify-between', isDone && "text-neutral-600")}>
        ...
        <div className='flex items-center gap-3'>
          ...
          <Button onClick={handleMarkSubtask} variant={"outline"} size={"sm"} className='text-xs'>Mark as completed</Button>
        </div>
    </motion.div>
  )
}
