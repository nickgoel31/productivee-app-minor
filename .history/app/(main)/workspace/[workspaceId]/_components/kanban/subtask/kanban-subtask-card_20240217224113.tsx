...
const KanbanSubtaskCard = ({subtask,workspaceId}:Props) => {
  ...
  return (
    <motion.div layout layoutId={subtask.id} className={cn('w-full lg:max-w-screen-lg border py-2 px-4 rounded flex items-center justify-between', isDone && "text-neutral-500")}>
        <div className='flex items-center gap-3'>
          <Checkbox onClick={handleMarkSubtask} className='opacity-80 border-neutral-500 block md:hidden'/>
        ...
        <div className='flex items-center gap-3'>
          ...
          <Button onClick={handleMarkSubtask} variant={"outline"} size={"sm"} className='text-xs hidden md:block'>
            {
