...
const KanbanColumn: React.FC<KanbanColumnProps> = ({ title, columnKey, headingColor, cards, setCards }) => {
    ...
    const handleDragMouseLeave = (e:DragEvent<HTMLDivElement>) => {
        e.preventDefault();
        setActive(false)
    }
    ...
    return (
        <div className='flex flex-col items-center gap-5 h-full min-h-[600px]'>
            ...
            <div onDragOver={handleDragOver} onDragLeave={handleDragMouseLeave} onDragEnd={handleDragEnd} className={`flex flex-col  items-center p-2 w-full h-full transition ${active ? "bg-neutral-900/50" : ""}`}>
