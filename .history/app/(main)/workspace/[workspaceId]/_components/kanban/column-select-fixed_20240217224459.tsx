...
const ColumnSelect = ({prevColumnId,taskId,workspaceId}:{prevColumnId:number,taskId:string,workspaceId:string}) => {
  ...
  return (
    <div className='flex w-full gap-2 '>
        {columns.map((col) => (
            <div 
            className={cn(`text-xs text-neutral-300 border rounded-full p-1.5 select-none cursor-pointer px-3 lowercase hover:bg-neutral-300/10 transition w-full text-center`, activeColumn === col.id && "bg-neutral-200 text-neutral-950 font-medium hover:bg-neutral-200/80")}
            key={col.id}
