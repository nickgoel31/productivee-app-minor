    ...
    return (
        <div className='flex flex-col items-center gap-5 h-full min-h-[600px]'>
            ...
            <div onDragOver={handleDragOver}
                onDragLeave={handleDragMouseLeave}
                onDrop={handleDragEnd}
            className={`flex flex-col  items-center gap-2 p-2 w-full h-full transition hover:bg-neutral-900/50 ${active ? "bg-neutral-900/50" : ""}`}>
                {tasks.map(task => (
                    <KanbanCard key={task.id} {...task} handleDragStart={handleDragStart}/>
                ))}
