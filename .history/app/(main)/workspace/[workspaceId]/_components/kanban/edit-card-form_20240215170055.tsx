...
const EditKanbanCardForm = ({prevTitle, workspaceId}:{prevTitle:string,workspaceId:string}) => {
    ...
    const handleEditTaskSubmit = (values: z.infer<typeof editTaskSchema>) => {
        values.tags = tags;
        editTaskInDB(values,workspaceId)
    }
                ...
                render={({field}) => (
                    <FormItem>
                        ...
                        <FormControl>
                            ...
                            <ToggleGroup type='multiple' className='flex items-end justify-start flex-wrap mid space-y-1'>
                                {defaultTags.map((tag) => (
                                    <ToggleGroupItem onClick={() => {handleTags(tag.value)}} value={tag.value} size={"sm"} className='text-xs text-neutral-300 font-medium border rounded-full px-3 capitalize'>{tag.value}</ToggleGroupItem>
                                ))}
                            </ToggleGroup>
                        </FormControl>
                        <FormMessage />
