...
const KanbanTaskPage = async ({params} : KanbanParams) => {
    ...
    const sortedSubtasks = subtasks?.sort((a,b) => a.priority < b.priority ? 1 : -1)

    const finalSortedSubtasks = sortedSubtasks?.sort((a, b) => {
    ...
    const isMoreThanOneTags = task.tags.length > 1 ? true : false;
    

  return (
    <ScrollArea className='w-full h-full py-1'>
      <div className='flex flex-col gap-4 pr-0 lg:pr-10 pt-6 pb-2 '>
        <div className='flex items-center w-full justify-between gap-4 h-full relative '>
            <div className='space-y-1 flex items-center gap-x-3'>
              <h2 className='text-2xl font-semibold'>{task.title}</h2>
              <div className='md:flex items-center gap-1 hidden'>
                {task.tags.slice(0, 3).map((tag,index) => (
                  <div key={index} className='text-xs font-medium p-1 px-3 rounded-full border capitalize'>
                    {tag}
                  </div>
                ))}
                {isMoreThanThreeTags && (
              ...
              <div className='md:hidden items-center gap-1 flex'>
                ...
                {isMoreThanOneTags && (
                  <HoverCard>
                    <HoverCardTrigger>
                        <p className='text-xs cursor-pointer'>+{task.tags.length - 1} more</p>
                    </HoverCardTrigger>
                    <HoverCardContent className="w-fit">
                      <div className='flex flex-col gap-2 items-start text-sm w-fit'>
                        <h4 className='font-semibold'>Tags</h4>
        ...
        <div className=''>
          ...
          <div className='fixed bottom-5 p-2 bg-background border rounded-full w-[95%] md:w-[60%] lg:w-[40%] lg:right-5 md:right-5 md:block pr-4 md:pr-0 flex items-center justify-center md:translate-x-0 z-10'>
      ...
      <div>
        ...
        <div className='w-full flex flex-col gap-7 md:px-3 px-0'>
          <div className='flex items-center justify-between gap-3 pr-0 lg:pr-10'>
            ...
            <div className='flex items-center gap-4'>
              ...
              {/* <MarkAllKanbanSubtaskBtn taskId={task.id}/> */}
            </div>
          </div>
