...
const WorkspaceIdPage = async ({params}:{params: IParams}) => {
  ...
  return (
    <ScrollArea className='h-full w-full pr-10 '>
      <div className='py-6 flex flex-col gap-7 relative h-full w-full '>
        ...
        <div className='flex flex-col w-full gap-4 items-center'>
          <h3 className='font-semibold text-xl'>My Todo Lists</h3>
          {!todos[0] && (
          ...
          <div className="flex flex-col gap-3">
                {sortedTodos.map((todo) => (
                    <TodoCard id={todo.id} key={todo.id} title={todo.title} isDone={todo.isDone} workspaceId={workspace.id} data={todo}/>
                ))}
          </div>
