...
export default async function Home() {
    ...
    const deleteAllIsDoneTodos = () => {
      for(let i=0; i<isDoneTodos.length;i++) {
        deleteAllTodoIsDone(isDoneTodos[i].id);
      }
    }
  ...
  return (
    <main className="flex flex-col gap-7">
        ...
        {isDoneTodos && (
          <div onClick={deleteAllIsDoneTodos} className="w-fit text-xs translate-y-[-10px] text-neutral-400 font-medium underline cursor-pointer transition hover:text-neutral-100">
